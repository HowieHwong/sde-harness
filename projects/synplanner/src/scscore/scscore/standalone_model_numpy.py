...
class SCScorer():
    ...
    def restore(self, weight_path=os.path.join(project_root, 'models', 'model.ckpt-10654.as_numpy.json.gz'), FP_rad=FP_rad, FP_len=FP_len):
...
if __name__ == '__main__':
    model = SCScorer()
    model.restore(os.path.join(project_root, 'models', 'model.ckpt-10654.as_numpy.json.gz'))
    ...
    model = SCScorer()
    model.restore(os.path.join(project_root, 'models', 'model.ckpt-10654.as_numpy.json.gz'), FP_len=2048)
    smis = ['CCCOCCC', 'CCCNc1ccccc1']
    for smi in smis:
        (smi, sco) = model.get_score_from_smi(smi)
        print('%.4f <--- %s' % (sco, smi))
