...
class Workflow:
    ...
    def __init__(
        ...
        if oracle is None:
            raise TypeError("oracle cannot be None")
        
        # Allow mock objects for testing, but validate real instances
        if not (isinstance(generator, Generation) or hasattr(generator, '_mock_name')):
            raise TypeError(f"generator must be an instance of Generation, got {type(generator).__name__}")
        if not (isinstance(oracle, Oracle) or hasattr(oracle, '_mock_name')):
            raise TypeError(f"oracle must be an instance of Oracle, got {type(oracle).__name__}")
