...
from sde_harness.core.oracle import (
    Oracle,
    improvement_rate_metric,
    consistency_metric,
    convergence_metric,
)
...
def example_1_basic_history_prompt():
    ...
    def stop_when_good_enough(context):
        ...
        return (
            scores.get("relevance", 0) >= 0.8 and scores.get("length_quality", 0) >= 0.9
        )
    ...
    result = workflow.run_sync(
        ...
        history_context={
            "task_description": "Scientific summarization with iterative improvement"
        },
    )

    # Check what keys are actually in the result
    if 'total_iterations' in result:
        ...
        print(f"Completed {len(result['history']['outputs'])} iterations")
    ...
    if 'final_scores' in result:
        ...
        print(f"Final scores: {result['history']['scores'][-1]}")
    
    if 'best_iteration' in result:
        print(
            f"Best iteration: {result['best_iteration']['iteration']} with score {result['best_iteration']['score']:.3f}"
        )
...
def example_2_dynamic_prompts():
    ...
    result = workflow.run_sync(
        ...
        history_context={
            "task_description": "Educational explanation with progressive enhancement"
        },
    )
    ...
    if 'final_scores' in result:
        ...
        print(f"Convergence achieved: {final_scores.get('convergence', 0):.3f}")
        print(f"Consistency achieved: {final_scores.get('consistency', 0):.3f}")
...
def example_3_conversation_style():
    ...
    conversation_prompt = Prompt(
        ...
        default_vars={"current_message": "Explain photosynthesis in simple terms"},
    )
    ...
    result = workflow.run_sync(
        ...
        history_context={
            "task_description": "Educational conversation about photosynthesis"
        },
    )

    print("--- Conversation Flow ---")
    for i, (prompt, output) in enumerate(
        zip(result["history"]["prompts"], result["history"]["outputs"]), 1
    ):
        print(f"Turn {i}:")
        # Extract the current message from the prompt
        if "Current message:" in prompt:
            message = (
                prompt.split("Current message:")[-1].split("Please respond")[0].strip()
            )
        ...
        for i in range(len(outputs)):
            for j in range(i + 1, len(outputs)):
                ...
                if len(words1) == 0 and len(words2) == 0:
                    ...
                    diversity = 1.0 - len(words1.intersection(words2)) / len(
                        words1.union(words2)
                    )
...
async def example_5_async_workflow():
    ...
    if 'total_iterations' in result:
        ...
        print(f"Async execution completed with {len(result['history']['outputs'])} iterations")
    ...
    if 'best_iteration' in result:
        ...
        print(f"Best output: {result['history']['outputs'][-1][:100]}...")
