...
def load_data(ligands_file: str = "data/1M-space_50-ligands-full.csv", 
              fitness_file: str = "data/ground_truth_fitness_values.csv") -> Tuple[str, pd.DataFrame, pd.DataFrame, Dict[str, int]]:
    """
    Load data files
...
def create_prompt(df_ligands_str: str, text_tmc: str, num_samples: int, num_provided_samples: int) -> Prompt:
    """
...
def setup_oracle(df_tmc: pd.DataFrame, tmc_samples: pd.DataFrame, mode: str = "multi_round") -> Oracle:
    """
    Setup oracle
    ...
    if mode == "multi_round":
        def improvement_rate_metric(history: dict, reference: any, current_iteration: int, **kwargs) -> float:
...
def validate_data_files(ligands_file: str = "data/1M-space_50-ligands-full.csv", 
                       fitness_file: str = "data/ground_truth_fitness_values.csv") -> bool:
    """
    Validate data files
    ...
    try:
        ...
        missing_ligand_cols = [col for col in required_ligand_cols if col not in df_ligands.columns]
        missing_tmc_cols = [col for col in required_tmc_cols if col not in df_tmc.columns]
